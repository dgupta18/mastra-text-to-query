...
export const databaseSeedingTool = createTool({
  ...
  execute: async ({ context: { connectionString, dbName, seedType, dropExisting, recordCount } }) => {
...
async function seedEcommerceData(db: Db, dropExisting: boolean, recordCount: number) {
  ...
  const products = [];
  const productNames = [
    'Smartphone', 'Laptop', 'Headphones', 'T-Shirt', 'Jeans', 'Novel', 'Cookbook',
    'Lamp', 'Chair', 'Basketball', 'Running Shoes', 'Tablet', 'Watch', 'Backpack'
  ];
  ...
  for (let i = 0; i < Math.min(recordCount * 1.5, 300); i++) {
    ...
    const order = {
      ...
      orderDate: new Date(Date.now() - Math.random() * 180 * 24 * 60 * 60 * 1000), // Last 6 months
      shippingAddress: {
...
async function seedSocialData(db: Db, dropExisting: boolean, recordCount: number) {
...
async function seedFinancialData(db: Db, dropExisting: boolean, recordCount: number) {
  ...
  for (let i = 0; i < Math.min(recordCount / 3, 100); i++) {
    const account = {
      ...
      currency: 'USD',
      openedAt: new Date(Date.now() - Math.random() * 1000 * 24 * 60 * 60 * 1000),
